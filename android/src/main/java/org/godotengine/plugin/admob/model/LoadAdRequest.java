						...
						if (network != null) {
							...
							if (extrasParamsObj instanceof Dictionary) {
								try {
									...
									Log.d(LOG_TAG, String.format("Processing %d extra parameters for %s", params.size(),
											networkTag));
									...
									for (String key : params.keySet()) {
										...
										if (val instanceof String) {
											...
											Log.d(LOG_TAG, String.format("Added ['%s',%s] extra for %s", key, val,
													networkTag));
										} else if (val instanceof Integer || val instanceof Double) {
											bundle.putDouble(key, ((Number) val).doubleValue());
											Log.d(LOG_TAG, String.format("Added ['%s',%.2f] extra for %s", key, val,
													networkTag));
										} else if (val instanceof Boolean) {
											bundle.putBoolean(key, (Boolean) val);
											Log.d(LOG_TAG, String.format("Added ['%s',%b] extra for %s", key, val,
													networkTag));
										} else if (val instanceof Long) {
											bundle.putLong(key, (Long) val);
											Log.d(LOG_TAG, String.format("Added ['%s',%d] extra for %s", key, val,
													networkTag));
										}
									}
									if (bundle.size() > 0) {
										...
										@SuppressWarnings("unchecked")
										Class<? extends Adapter> adapterClass = (Class<? extends Adapter>)
												Class.forName(network.getAdapterClassName());
									...
									Log.w(LOG_TAG, "Class not found for adapter: " + network.getAdapterClassName()
									...
									Log.e(LOG_TAG, "Error adding extras for " + network.getAdapterClassName()
											+ ": " + e.getMessage());
								}
							} else {
								Log.w(LOG_TAG, String.format("Invalid %s entry: Missing '%s' or '%s'. Skipping.",
										NETWORK_EXTRAS_PROPERTY, NETWORK_TAG_SUBPROPERTY, EXTRAS_SUBPROPERTY));
							}
						} else {
